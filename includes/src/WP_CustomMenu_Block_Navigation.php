    ...
    public function drawCustomMenuItem($category, $level = 0, $last = false)
    {
        ...
        if ($drawPopup) {
            ...
            if (count($activeChildren)) {
                ...
                $htmlPopup[] = '<div class="block1"><h4>Shop by Category</h4>';
    ...
    public function drawMenuItem($children, $level = 1)
    {
        ...
        foreach ($children as $child) {
            ...
            if (is_object($child) && $child->getIsActive()) {
                ...
                $html.= '<li class="level' . $level .'"><a class="itemMenuName level' . $level . $active . '" href="' . $this->getCategoryUrl($child) . '">' . $name . '</a></li>';
				if($GLOBALS['count'] == 8 ){
                	$html.='</ul>';
					$GLOBALS['count']=0;
                }
